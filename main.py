...
async def show_category(query, category):
    ...
    for product_id, _, _, duration, price, stock, active in products:
        ...
        keyboard.append([InlineKeyboardButton(label, callback_data=f"product:{product_id}")])
...
async def create_order(query, product_id):
    try:
        order_id, name, duration, price = db.create_order(query.from_user.id, product_id)
    ...
    # The order stays pending until a real gateway verification is implemented.
    await query.edit_message_text(
        f"✅ سفارش #{order_id} ایجاد شد.\n\n"
        f"📦 محصول: {name}\n📅 مدت: {duration} ماه\n💰 مبلغ: {format_price(price)}\n\n"
        "⏳ وضعیت: در انتظار پرداخت\n\n"
        "درگاه زرین‌پال در مرحله اتصال کلیدهای حساب فعال می‌شود.",
...
async def show_support(query):
    context = query._bot if False else None
    await query.edit_message_text(
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "shop":
            await query.edit_message_text("🛒 فروشگاه\n\nمحصول موردنظر را انتخاب کنید:", reply_markup=shop_menu())
            ...
            await query.edit_message_text("🔐 کانفیگ VLESS را ارسال کن.\n\nهر پیام فقط یک کانفیگ باشد.")
...
async def show_admin_products(query):
    ...
    for pid, category, name, duration, price, stock, active in products:
        ...
        keyboard.append([InlineKeyboardButton(f"{cat} {duration} ماهه | {format_price(price)}", callback_data=f"admin_price:{pid}")])
        if category != "vpn":
            keyboard.append([InlineKeyboardButton(f"📦 موجودی: {stock}", callback_data=f"admin_stock:{pid}")])
    keyboard += [
...
async def show_admin_orders(query):
    ...
    if not orders:
        await query.edit_message_text("🧾 هنوز سفارشی ثبت نشده.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 پنل مدیریت", callback_data="admin")]]))
        return
    keyboard = []
    for oid, user_id, name, duration, price, status, created_at in orders:
        keyboard.append([InlineKeyboardButton(f"{status_name(status)} #{oid} | {format_price(price)}", callback_data=f"admin_order:{oid}")])
    keyboard.append([InlineKeyboardButton("🔙 پنل مدیریت", callback_data="admin")])
    await query.edit_message_text("🧾 سفارش‌های اخیر:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_admin_order(query, order_id):
    ...
    if status == "pending_payment":
        keyboard.append([
            InlineKeyboardButton("✅ تأیید/تحویل", callback_data=f"approve:{oid}"),
...
async def admin_approve(query, order_id):
    ...
    if result != "OK":
        messages = {"NOT_FOUND": "سفارش پیدا نشد.", "NOT_PENDING": "این سفارش قبلاً پردازش شده.", "OUT_OF_STOCK": "کانفیگ VLESS موجود نیست."}
    ...
    if order:
        ...
        if delivery:
            try:
                await query.get_bot().send_message(chat_id=user_id, text=f"✅ سفارش #{order_id} تأیید شد.\n\n🔐 کانفیگ VLESS:\n\n{delivery}")
            except Exception:
...
async def admin_reject(query, order_id):
    ...
    _, user_id = result
    try:
        await query.get_bot().send_message(chat_id=user_id, text=f"❌ سفارش #{order_id} رد شد.")
    except Exception:
...
async def show_inventory(query):
    ...
    text = "🔐 هیچ کانفیگی ثبت نشده است." if not inventory else "🔐 موجودی VLESS:\n\n" + "\n".join(f"#{cid} | {duration} ماه | {status}" for cid, name, duration, status, created in inventory)
...
async def admin_value_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not product_id:
        return ConversationHandler.END
    value = int(text)
    db.update_product_value(product_id, edit_type, value)
    message = f"✅ قیمت جدید: {format_price(value)}" if edit_type == "price" else f"✅ موجودی جدید: {value}"
    context.user_data.clear()
    await update.message.reply_text(message, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📦 مدیریت محصولات", callback_data="admin_products")]]))
    return ConversationHandler.END
...
async def vpn_config_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return WAIT_VPN_CONFIG
    context.user_data.clear()
    await update.message.reply_text(f"✅ کانفیگ VLESS با شناسه #{cid} ثبت شد.", reply_markup=admin_keyboard())
    return ConversationHandler.END
...
async def support_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not message:
        return
    db.add_support_message(user.id, message)
    context.user_data.clear()
    await update.message.reply_text("✅ پیام شما برای پشتیبانی ارسال شد.", reply_markup=main_menu())
    if ADMIN_ID:
        try:
            await context.bot.send_message(chat_id=int(ADMIN_ID), text=f"📞 پیام پشتیبانی جدید\n\n👤 {user.id}\n\n{message}")
        except Exception:
...
def main():
    ...
    admin_value_conversation = ConversationHandler(
        ...
        states={WAIT_ADMIN_VALUE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_value_handler)]},
        fallbacks=[]
    )
    ...
    vpn_conversation = ConversationHandler(
        ...
        states={WAIT_VPN_CONFIG: [MessageHandler(filters.TEXT & ~filters.COMMAND, vpn_config_handler)]},
        fallbacks=[]
    )
