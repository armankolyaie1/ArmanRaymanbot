...
async def show_account(query):
    orders = await db.get_user_orders(query.from_user.id, limit=1000)
    await query.edit_message_text(
        "👤 حساب شما\n\n"
...
async def show_support(query):
    # Conversation state is set by the callback handler.
    await query.edit_message_text(
        "📞 پشتیبانی\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await db.save_user(query.from_user)
    data = query.data
    ...
    try:
        if data == "shop":
            ...
            if is_admin(query.from_user.id):
                context.user_data["reply_support_id"] = int(data.split(":")[1])
                await query.edit_message_text("✉️ پاسخ را ارسال کن:")
                return WAIT_SUPPORT
    except Exception:
    ...
    if order:
        ...
        try:
            if delivery:
                ...
                text = (
                    f"✅ سفارش #{order_id} تأیید شد.\n\n"
                    "📱 محصول شما تأیید شد و تحویل آن طبق روش فروش شما انجام می‌شود."
                )
            await query.get_bot().send_message(chat_id=user_id, text=text)
        except Exception:
            logger.exception("Delivery notification failed")

    await query.answer("سفارش تأیید شد.", show_alert=True)
...
async def admin_value_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    value = int(text)
    if edit_type == "price":
        await db.update_product_value(product_id, "price", value)
        message = f"✅ قیمت جدید: {format_price(value)}"
    else:
        await db.update_product_value(product_id, "stock", value)
...
async def vpn_config_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    product_id = context.user_data.get("vpn_product_id")

    if not product_id:
        await update.message.reply_text("❌ محصول VLESS مشخص نیست.")
        return ConversationHandler.END

    try:
        cid = await db.add_vpn_config(product_id, config)
    except ValueError:
        await update.message.reply_text("❌ کانفیگ خالی است.")
        return WAIT_VPN_CONFIG
    except Exception:
        logger.exception("Could not add VLESS config")
        await update.message.reply_text("❌ ثبت کانفیگ انجام نشد؛ احتمالاً تکراری است.")
        return WAIT_VPN_CONFIG
...
async def support_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("reply_support_id") and is_admin(user.id):
        support_id = context.user_data["reply_support_id"]
        reply = update.message.text.strip()
        if reply:
            result = await db.reply_support_message(support_id, reply)
            if result:
                user_id = result
                try:
                    ...
                    logger.exception("Support reply delivery failed")
            context.user_data.clear()
            await update.message.reply_text(
                "✅ پاسخ ارسال شد.",
                reply_markup=admin_keyboard(),
    ...
    await db.create_support_message(user.id, message)
    context.user_data.clear()
        ...
        try:
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "✉️ پاسخ",
                        callback_data=f"admin_support_reply:{await db.latest_support_id(user.id)}"
                    )]
                ]),
            )
        except Exception:
...
async def show_admin_support(query):
    ...
    if not messages:
        text = "📞 پیام پشتیبانی باز وجود ندارد."
        ...
        for sid, user_id, message, status, created_at in messages:
            lines.append(f"#{sid} | 👤 {user_id}\n{message}\n")
            keyboard.append([
                InlineKeyboardButton(
                    f"✉️ پاسخ #{sid}",
                    callback_data=f"admin_support_reply:{sid}"
                )
            ])
        keyboard.append([InlineKeyboardButton("🔙 پنل مدیریت", callback_data="admin")])
        text = "\n".join(lines)
...
async def admin_support_reply_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END

    support_id = context.user_data.get("reply_support_id")
    if not support_id:
        return ConversationHandler.END
    ...
    user_id = await db.reply_support_message(support_id, reply)
    if user_id:
        try:
            await context.bot.send_message(
                chat_id=user_id,
...
async def show_admin_stats(query):
    users, orders, approved, revenue, vpn_available = await db.stats()
    await query.edit_message_text(
        "📊 آمار فروشگاه\n\n"
        f"👥 کاربران: {users}\n"
        f"🧾 سفارش‌ها: {orders}\n"
...
async def error_handler(update, context):
    logger.error("Unhandled error: %r", context.error, exc_info=context.error)
...
def main():
    ...
    admin_value_conversation = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(button_handler, pattern=r"^admin_(price|stock):")
        ],
        states={
            WAIT_ADMIN_VALUE: [
    ...
    application.add_handler(admin_value_conversation)

    vpn_conversation = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(button_handler, pattern=r"^admin_add_vpn:")
        ],
        states={
            WAIT_VPN_CONFIG: [
    ...
    application.add_handler(vpn_conversation)

    support_reply_conversation = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(button_handler, pattern=r"^admin_support_reply:")
        ],
        states={
            WAIT_SUPPORT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin_support_reply_handler)
            ]
        },
        fallbacks=[],
        allow_reentry=True,
    )
    application.add_handler(support_reply_conversation)

    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, support_handler)
    )
