...
def init_db():
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            # Keep product stock synchronized with available VPN configs.
            cur.execute("""
                UPDATE products p
                SET stock = (
                    SELECT COUNT(*)
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                WHERE FALSE
            """)  # harmless no-op for compatibility with older schemas
...
def get_user_orders(user_id, limit=20):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    o.id, p.name, p.duration, o.price, o.status,
                    o.created_at
                FROM orders o
...
def get_order(order_id):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    o.id, o.user_id, o.product_id, p.category, p.name,
                    p.duration, o.price, o.status, o.created_at
                FROM orders o
                JOIN products p ON p.id = o.product_id
                WHERE o.id = %s
            """, (order_id,))
...
def add_vpn_config(product_id, config):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO vpn_configs (product_id, config)
                VALUES (%s, %s)
                RETURNING id
            """, (product_id, config))
...
def get_vpn_inventory():
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    v.id, p.name, p.duration, v.status, v.created_at
                FROM vpn_configs v
...
def get_admin_orders(limit=30):
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    o.id, o.user_id, p.name, p.duration,
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            if field == "price":
                ...
                # Stock for VPN is derived from configs; don't manually overwrite it.
                cur.execute("""
                    UPDATE products
                    SET stock = %s
                    WHERE id = %s AND category <> 'vpn'
                """, (value, product_id))
            else:
                cur.execute("UPDATE products SET active = %s WHERE id = %s",
                            (value, product_id))
        conn.commit()
...
def stats():
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT COUNT(*) FROM vpn_configs WHERE status = 'available'
            """)
            vpn_available = cur.fetchone()[0]
